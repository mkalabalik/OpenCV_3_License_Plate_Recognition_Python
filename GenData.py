...
###################################################################################################
def main(imgTraining = None):
    c = open("classifications.txt", 'a')          # open files 
    f = open("flattened_images.txt", 'a')

    if imgTraining is not None :                               # choose file which will be used for training chars
        imgTrainingNumbers = cv2.cvtColor(imgTraining, cv2.COLOR_GRAY2BGR)           # from arg from main function
        imgThresh = cv2.bitwise_not(imgTraining)
    else:
        fileTraining = "training_chars.png"       # from directory

        imgTrainingNumbers = cv2.imread(fileTraining) #training_chars_1.png")            # read in training numbers image
    ...
    np.savetxt(c, npaClassifications)           # write flattened images to file
    np.savetxt(f, npaFlattenedImages)          #

    cv2.destroyAllWindows()             # remove windows from memory

    c.close()                           # close files at the end
    f.close()
    return
